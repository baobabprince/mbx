...
test2_first_p <- test2_first %>% pivot_longer(cols = contains("area"), names_to = NULL, values_to = "Y")
...
pl1 <-
  ...
  ggplot(aes(x=Visit, y=Peak_Area, color = Disease_Status, group = pn_ID)) +
  geom_point(size = 3) + 
...
pl1 <-
  data3 |> 
  filter(feature %in% first_metabolites) |> 
  ggplot(aes(x=Visit, y=Peak_Area, color = Disease_Status)) +
  geom_point(size = 3) + 
  geom_line(group = pn_ID, color = "gray", alpha = .5) +
  geom_blank(data = test2_first_p, aes(x = X, y = Y)) +
  ggh4x::facet_nested(IIRN_Flare ~ feature, scales = "free_y") +
  scale_x_continuous(breaks = seq(from = 1,to = 6,by = 1)) + 
  #scale_y_continuous(limits = ) +
  #scale_y_continuous(limits = c(0,rep(round(as.numeric(y_plot_range$area_max), digits=2),each = 2))) +
  #scale_y_continuous(breaks = testfunc) + #my_breaks, labels = my_labels) +
  theme(legend.position = "none")
