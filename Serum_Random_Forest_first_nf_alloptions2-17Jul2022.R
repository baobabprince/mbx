...
data_source <- 'MZPos_N1_filtered'   #'_MZPos-N1_biom2'   #'_MZNeg-N1_biom2'    #'MZM10_N2_filtered' # nolint
data_source <-"MZM10_N2_filtered"
samples_type <- 'Serum'
# samples_type <- 'Stool'
check_val = 'IIRN_Flare'
na_str = c('no_data','_','NA','unknown', 'other','na') # nolint

map_file <- paste0(samples_type,'_metadata.txt')
data_file <- paste0(samples_type,'_Data_',data_source,"_calour.tsv")  ## For untargeted files
# data_file <- paste0(samples_type,'_Data_',data_source,"_calour.tsv")  ## For targeted files

map0 <- paste0(map_file) |> read_tsv()
map0$SampleID <- make.names(map0$SampleID)

data1 <- paste0(data_file) |> read.table(sep="\t", na.strings = na_str, row.names = 1) |> t() |> as.data.frame()
